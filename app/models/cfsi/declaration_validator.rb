...
private
  def validate_base_fields(declaration)
    @base << @messages[:declaration][:no_presence][:company_name] if declaration.company_name.to_s.empty?
    @base << @messages[:declaration][:no_presence][:declaration_scope] if declaration.declaration_scope.to_s.empty?
    @base << @messages[:declaration][:no_presence][:authorized_company_representative_name] if declaration.authorized_company_representative_name.to_s.empty?
    @base << @messages[:declaration][:no_presence][:representative_email] if declaration.contact_email.to_s.empty?
    @base << @messages[:declaration][:no_presence][:completion_at] if declaration.completion_at.to_s.empty?
    @base << "(%{declaration.language}): " + @messages[:declaration][:no_presence][:language] if declaration.language.to_s.empty?
  end
