class Trial::TrialUser < User
  ...
  def email_domain_cannot_exist
    ...
    if domain != "greenstatuspro.com" &&
      !Trial::TrialUser.where("email LIKE ?", "%@#{domain}").where((self.id) ? "id <> ?" : "id IS NOT ?", self.id).empty?
      errors.add(:email, "domain '#{domain}' has been previously registered") unless errors.full_messages.include?("Email has already been taken")
      return false
    end
  end
end
