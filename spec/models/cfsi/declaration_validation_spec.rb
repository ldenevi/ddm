...
CFSI_XLS_PATH     = File.join(File.dirname(__FILE__), "sample_cmrts")
...
describe Cfsi::Declaration do
  ...
  context "during empty CFSI template submission" do
    VERSIONS.each do |version|
      let (:declaration) { Cfsi::Declaration.generate File.join(CFSI_XLS_PATH, version, "#{version}_-_empty.xls") }
  ...
  context "during valid CFSI template submission #1" do
    VERSIONS.each do |version|
      let (:declaration) { Cfsi::Declaration.generate File.join(CFSI_XLS_PATH, version, "#{version}_-_valid_1.xls") }
  ...
  context "during valid CFSI template submission #2" do
    VERSIONS.each do |version|
      let (:declaration) { Cfsi::Declaration.generate File.join(CFSI_XLS_PATH, version, "#{version}_-_valid_2.xls") }
